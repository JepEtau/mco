...
class Model_viewer(QObject):
    ...
    def directory_changed(self, values:dict):
        ...
        # print("\n-------------------\ndirectory_changed: %s:%s" % (k_ep, k_part))
        # pprint(values)
        ...
        if 'hide' in values.keys():
            self.update_filter_by(values['hide'])
        ...
        new_widget_values = {
            ...
            'reload': True if 'reload' in values.keys() else False
        }
        # print("\ndirectory_changed, result")
