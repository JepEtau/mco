...
class Widget_selection(QWidget, Ui_widget_selection):
    ...
    def __init__(self, ui, model):
        ...
        headers = ["shot", "src", "start", "count", "curves"]
        default_col_width = [50, 60, 65, 60, 40, 120]
    ...
    def event_refresh_shotlist(self, values:dict):
        ...
        if values['geometry'] is not None:
            self.lineEdit_crop_coordinates.setText("(%s)" % (', '.join(map(lambda x: "%s" % (x), values['geometry']))))
        else:
            self.lineEdit_crop_coordinates.clear()
        ...
        for k_shot, shot in shots.items():
            ...
            self.tableWidget_shots.setItem(row_no, 4, QTableWidgetItem(k_curves))
            if not shot['is_valid']:
                self.tableWidget_shots.item(row_no, 0).setForeground(QBrush(QColor(255, 0, 0)))
