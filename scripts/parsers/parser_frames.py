    ...
    for k_section in config.sections():
        ...
        if k_section == 'frames':
            ...
            for k_part in config.options(k_section):
                ...
                nested_dict_set(db_frames, list(), k_part, 'list')
                parse_framelist(db_frames[k_part]['list'], value_str)
...
def parse_frames_for_study_g(db, k_part_g):
    ...
    for k_section in config.sections():
        ...
        if k_section == 'frames':
            # Parse this section only when in study mode (--frames)
            db_frames['path_output'] =  os.path.join(db['common']['directories']['frames'], f"{k_part_g}")
