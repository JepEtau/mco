...
def parse_video(db_video, config, verbose=False):
    ...
    for k_option in config.options(k_section):
        ...
        # Start timestamp
        result = re.search(re.compile("(\d+)"), properties[0])
        if result is not None:
            start = int(result.group(1))
            # will be sync with audio if start equals to -1
        else:
            sys.exit("error: video section in common")

        # End timestamp
        result = re.search(re.compile("(-?\d+)"), properties[1])
        if result is not None:
            end = int(result.group(1))
            # will be sync with audio if end equals to -1
        else:
            sys.exit("error: video section in common")
        ...
        if len(properties) > 1:
            ...
            for i in range(1, len(properties)):
                # print("search keyword in [%s]" % (properties[i]))
                search_fadein = re.search(re.compile("fadein=([0-9.]+)"), properties[i])
                if search_fadein is not None:
                    search_fadein = int(float(search_fadein.group(1)) * FPS)
                    continue

                search_fadeout = re.search(re.compile("fadeout=([0-9.]+)"), properties[i])
                if search_fadeout is not None:
