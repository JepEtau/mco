...
def nested_dict_merge(dict1, dict2):
    for k1, v1 in dict1.items():
        if type(v1) == dict:
            if k1 in dict2.keys() and type(dict2[k1]) == dict:
                nested_dict_merge(dict1[k1], dict2[k1])
        else:
            if k1 in dict2:
                dict1[k1] = dict2[k1]
...
def get_shot_from_frame(db, k_ed:str, frame:dict, k_part:str=''):
    ...
    print("common.get_shot_from_frame")
    ...
    db_video = db[k_episode]['video'][k_ed][k_part]
    # if 'shots' not in db_video.keys():
    #     print("USE REFERENCE, correct this")
    #     db_video = db[k_episode]['video'][...][k_part]
    #     frame_no = frame['ref']
    # else:
    #     frame_no = frame['no']
...
def get_shot_from_frame_no_new(db, frame_no:int, k_ed, k_ep, k_part) -> dict:
    ...
    shots = db[k_ep_ref]['video'][k_ed_ref][k_part]['shots']
    for shot in shots:
        # print("%d in [%d; %d] ?" % (frame_no, shot['start'], shot['start'] + shot['count']))
    ...
    pprint_video(db[k_ep_ref]['video'][k_ed_ref][k_part], ignore='')
    print("-----------------------------------------------")
...
def get_or_create_src_shot(db, frame_no:int, k_ed, k_ep, k_part)-> dict:
    ...
    is_found = False
    if 'shots' in db[k_ep]['video'][k_ed][k_part]:
        shots = db[k_ep]['video'][k_ed][k_part]['shots']
        for shot in shots:
            if shot is None:
                continue
    ...
    if k_part in K_GENERIQUES:
        ...
        try:
            k_ed_src = db[k_ep]['video']['target']['src']['k_ed']
        except:
        ...
        try:
            k_ep_src = db[k_ep]['video']['target']['src']['k_ep']
        except:
            k_ep_src = k_ep
    ...
    is_found = False
    shots = db[k_ep_src]['video'][k_ed_src][k_part]['shots']
    for shot in shots:
        if shot['start'] <= frame_no < (shot['start'] + shot['count']):
    ...
    if not is_found:
        ...
        # pprint(db[k_ep_src]['video'][k_ed_src][k_part])
        return None
...
# async def create_async_pipe(command, images, bufsize=10**8):
...
#         pipe = await asyncio.create_subprocess_exec(command,
