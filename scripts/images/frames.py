...
from filters.utils import get_filters_from_shot
...
def consolidate_frame_list_for_study(db, k_ed, k_ep, k_part, tasks, force:bool=False) -> list:
    ...
    for frame in frame_list:
        ...
        if k_ed_src != k_ed_ref or k_ep_src != k_ep_ref:
            print("\tconvert frame_no into frame_ref, ref = %s:%s" % (k_ed_ref, k_ep_ref), end='')
            ...
            if 'offsets' in db[k_ep_src][k_ed_src][k_part]['video']:
                ...
                for offset in offsets:
                    if offset['start'] <= (frame['no'] - start)<= offset['end']:
                        ...
                        # print("\tfound offset: %d in[%d:%d]" % (offset['offset'], offset['start'], offset['end']))
                        break
            else:
                # print("No offset:")
                frame['ref'] = frame['no']
        ...
        frame.update({
            'curves': shot['curves'],
            'tasks': tasks.copy(),
            # Repair this as input has been removed from 'video' struct
